...
#    this is x <-|    |-> this is y
mtcars |> plot2(mpg, hp)
# y can contain multiple variables, and even supports Tidyverse selection helpers:
...
```{r}
# this uses the whole viridis colour scale:
options(plot2.colour_sf_fill = "viridis")
plot2(netherlands)
# but this uses only the first viridis colour:
options(plot2.colour_sf_fill = colourpicker("viridis", length = 1))
plot2(netherlands)
```
