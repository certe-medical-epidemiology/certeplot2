...
The scale for the `category` is also something that can be determined automatically quite easily - it is after all based on the input data. `ggplot2` returns an error if the scale does not match the data:

```{r, error = TRUE, fig.height = 0}
...
So updating the `aes()` part, also requires users to update the `scale_fill*()` part in this case. `plot2()` determines the type of scale automatically:
