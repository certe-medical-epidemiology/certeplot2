...
The `ggplot2` package in conjunction with the `tidyr`, `forcats` and `cleaner` packages can provide above functionalities, but the goal of the [plot2()] function is to generalise this into one function. The default `plot2()` function currently has `r length(formals(plot2)) - 1` arguments, all with a default value. **Less typing, faster coding.**
...
The `ggplot2` package must (of course) be installed, but it does not need to be loaded using `library()`. The same goes for other tidyverse packages, such as `dplyr`, `forcats` and `tidyr`. They are only used internally and not required to load before using the `plot2()` function.
