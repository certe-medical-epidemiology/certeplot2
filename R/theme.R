...
theme_minimal2 <- function(subtitle.colour = "black",
  ...
  t <- theme_bw(base_size = 11 * font.size_factor) %+replace%
    theme(
      ...
      # axis.text.y = element_text(margin = margin(l = 10 * font.size_factor,
      #                                            r = -21 * font.size_factor, unit = "pt"),
      #                            hjust = 0,
      #                            vjust = -0.5),
      ...
      legend.key.size = unit(11 * font.size_factor, "pt"), # blokjes en lijnen links van tekst in legenda
      legend.text = element_text(size = unit(9 * font.size_factor, "pt"), # tekst zelf
                                 margin = margin(l = 1, r = 6, unit = "pt"), # ruimte links en rechts van tekst
                                 face = legend.italic),
      legend.position = legend.position,
      legend.title = element_text(face = "bold", size = unit(10 * font.size_factor, "pt")),
      ...
      plot.subtitle = element_text(size = unit(11 * font.size_factor, "pt"),
                                    margin = margin(0, 0, ifelse(has_subtitle == TRUE, 15, 7), 0),
                                    hjust = 0.5,
                                    colour = subtitle.colour),
      plot.title = element_text(size = unit(13 * font.size_factor, "pt"),
                                 margin = margin(0, 0, ifelse(has_subtitle == TRUE, 7, 15), 0),
                                 hjust = 0.5,
                                 colour = "black"),
      plot.caption = element_text(colour = "grey50",
                                   size = unit(10 * font.size_factor, "pt"),
                                   hjust = 1),
      plot.tag = element_text(size = unit(14 * font.size_factor, "pt"),
      ...
      strip.text = element_text(face = case_when(facet.bold & facet.italic ~ "bold.italic",
                                                 ...
                                                 facet.italic ~ "italic",
                                                 TRUE ~ "plain"),
                                size = unit(facet.size * font.size_factor, "pt"),
                                margin = margin(t = facet.margin, b = facet.margin / 2)),
      strip.switch.pad.wrap = unit(10 * font.size_factor, "pt"),
