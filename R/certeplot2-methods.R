...
plot2.antibiogram <- function(.data,
  ...
  df <- attributes(.data)$out_numeric
  if (is.null(df)) {
    # TODO remove this part later, it's only to support old version of AMR (<3.0)
    df <- attributes(.data)$long
  } else {
    df$SI <- df$numerator / df$total
  }
  ...
  y_scale <- validate_y_scale(df = df |> mutate(`_var_y` = SI),
                              ...
                              y.percent = y.percent,
                              y.percent_break = y.percent_break,
                              misses_y.percent_break = missing(y.percent_break),
                              y.position = y.position,
                              y.transform = "identity",
                              y.zoom = FALSE,
                              stacked = FALSE,
